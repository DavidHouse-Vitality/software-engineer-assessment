...
public class HomeSearchFacadeTests
{
     ...
     */
    [Fact]
    public static async Task HomeSearchFacade_EmptyString_NoPropertiesReturned()
    {
    ...
    private static Mock<IHomeSearchClient> GetMockClient()
    {
        ...
        clientMock.Setup(c => c.GetPolicies()).ReturnsAsync(TestPolicies);
        clientMock.Setup(c => c.GetPolicy(It.IsAny<int>()))
            .ReturnsAsync((int id) => TestPolicies.Find(p => p.Id == id));
        clientMock.Setup(c => c.GetClaims()).ReturnsAsync(TestClaims);
        clientMock.Setup(c => c.GetClaimsByPolicyId(It.IsAny<int>()))
            .ReturnsAsync((int policyId) => TestClaims.FindAll(c => c.PolicyId == policyId));
        clientMock.Setup(c => c.GetClaim(It.IsAny<string>())).ReturnsAsync((string claimNumber) =>
            TestClaims.Find(c => c.ClaimNumber == claimNumber));
        return clientMock;
    }
}